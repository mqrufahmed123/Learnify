...
// GET /api/quiz/attempt/:attemptId  — full review for a past attempt
const getQuizAttemptDetails = asyncHandler(async (req, res) => {
  const { attemptId } = req.params;

  const attempt = await QuizAttempt.findOne({ _id: attemptId, userId: req.user.id });
  if (!attempt) return res.status(404).json({ message: 'Attempt not found' });

  const quiz = await Quiz.findById(attempt.quizId);
  if (!quiz) return res.status(404).json({ message: 'Original quiz not found' });

  // Rebuild review by matching saved answers against quiz questions
  const review = quiz.questions.map((q, i) => {
    const saved = attempt.answers.find(a => a.questionIndex === i);
    return {
      questionText: q.questionText,
      options: q.options,
      correctAnswer: q.correctAnswer,
...
module.exports = { generateQuiz, submitQuiz, getQuizHistory, getQuizAttemptDetails };
